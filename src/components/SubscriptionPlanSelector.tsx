...
export const SubscriptionPlanSelector: React.FC<SubscriptionPlanSelectorProps> = ({
  ...
  const formatPrice = (monthlyPrice: number, yearlyPrice: number, currency = 'USD') => {
    ...
    if (billingCycle === 'yearly') {
      ...
      return {
        price: `${currencySymbol}${monthlyEquivalent.toLocaleString('es-CO', {
          maximumFractionDigits: 0
        })}`,
        period: '/mes',
        savings: monthlyPrice > monthlyEquivalent ? `Ahorra ${currencySymbol}${((monthlyPrice - monthlyEquivalent) * 12).toLocaleString('es-CO', {
          maximumFractionDigits: 0
        })} al año` : null
      };
    }
    return {
      price: `${currencySymbol}${monthlyPrice.toLocaleString('es-CO', {
        maximumFractionDigits: 0
      })}`,
      period: '/mes',
      savings: null
  ...
  if (isCreating) {
    return <AILegalPaymentLoader stage="processing" message="Configurando tu suscripción con dLocal..." progress={25} />;
  }
  if (isLoading) {
    console.log('SubscriptionPlanSelector: Loading plans...');
    return <div className={`grid gap-6 md:grid-cols-2 lg:grid-cols-3 ${className}`}>
  ...
  if (!isLoading && plans.length === 0) {
    return <div className={`text-center p-8 ${className}`}>
  ...
  return <>
      <LawyerTermsAndConditions open={showTerms} onAccept={handleTermsAccept} onDecline={handleTermsDecline} />
      ...
      <div className={`min-h-screen bg-gradient-to-br from-background via-muted/30 to-background py-8 px-4 sm:px-6 lg:px-8 ${className}`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid gap-6 sm:gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-8">
            ...
            return <Card key={plan.id} className={`relative h-full flex flex-col transition-all duration-300 hover-lift-subtle ${isPopular ? 'border-2 border-primary shadow-elevated scale-105' : 'border border-border hover:border-primary/50'}`}>
                  {isPopular && <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-10">
                  ...
                  <CardHeader className="text-center pb-6 pt-8">
                    <div className="flex justify-center mb-4">
                      <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full ${isPopular ? 'bg-primary/10' : 'bg-muted'}`}>
                    ...
                    <div className="mt-6 mb-2">
                      ...
                      {pricing.savings && <div className="mt-2 inline-block px-3 py-1 bg-green-500/10 rounded-full">
                ...
                }} size="lg" className={`w-full transition-all duration-300 font-semibold ${isPopular ? 'bg-primary hover:bg-primary-light shadow-lg hover:shadow-xl hover:scale-105' : isFree ? 'bg-secondary hover:bg-secondary-dark text-secondary-foreground' : 'bg-muted hover:bg-muted/80 text-foreground hover:shadow-md'}`} disabled={isLoading || isCreating}>
                      {isLoading || isCreating ? <div className="flex items-center gap-2">
                          <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
                          <span>Procesando...</span>
                        </div> : isFree ? 'Comenzar Gratis' : 'Seleccionar Plan'}
                    </Button>
                  </CardContent>
                </Card>;
          })}
