-- Función para enviar correo de bienvenida cuando el abogado confirma su email
  ...
  IF OLD.email_confirmed_at IS NULL AND NEW.email_confirmed_at IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      IF FOUND THEN
        ...
        -- Nota: El edge function 'send-email' debe ser invocado por otro proceso
        -- o podemos usar pg_net para hacer la llamada HTTP aquí
...
DROP TRIGGER IF EXISTS on_lawyer_email_confirmed ON auth.users;
CREATE TRIGGER on_lawyer_email_confirmed
  AFTER UPDATE ON auth.users
  FOR EACH ROW
  EXECUTE FUNCTION public.send_lawyer_welcome_email();